...
# - Achieves ~ 94% classification rate on a [100,50] architecture over 1000 iterations in 1 minute
# - Can display classification rate during training but is much slower than displaying error rates
...
# - Added feature to save model parameters if performance exceeds 93% on test data
#
...
nodes = [100,50] # Neural network architecture [x1,x2,..] where xi is the number of nodes in layer i
k = len(nodes)
nu = 0.1 # learning rate
gamma = 0.0001 # adaptive learning rate hyperparameter / nu decay
...
r = 0.001
index_batch = np.random.choice([True,False],X_train.shape[0],p=[r,1-r])
X_batch =  X_train[index_batch]
y_batch = y_train[index_batch]

# Construct weight matrices 
W_size = 0.02 # scales down variance of initial weights
...
while iteration <= 1000:
	...
	if track_validation == True:
		...
		temp = np.tanh(X_validate.dot(W[0]))
		temp = np.append(np.ones((temp.shape[0],1)),temp,axis = 1) # Append bias node
		y_validate_hat = temp

		for i in range(1,k):
			temp = np.tanh(y_validate_hat.dot(W[i]))
			temp = np.append(np.ones((temp.shape[0],1)),temp,axis = 1) # Append bias node
			y_validate_hat = temp

		y_validate_hat = np.tanh(y_validate_hat.dot(W[k]))
		error_validate = 0.5 * (y_validate_hat - y_validate).T.dot(y_validate_hat - y_validate)
		errors_validation.append(error_validate[0,0])

		# Display errors
		print "Iteration:", iteration,"in-error:",error[0,0],"out-error:",error_validate[0,0]
	...
	if track_classification == True:
		...
		y_validate_hat = np.tanh(y_validate_hat.dot(W[k]))
		y_validate_hat_predict = hardmax(y_validate_hat)
...
y_test_hat_predict = hardmax(y_test_hat)
...
# Save model if performance > 93% 
if model_performance > 93.:
